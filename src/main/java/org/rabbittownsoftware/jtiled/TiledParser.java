...
public class TiledParser
{
    ...
    /**
     * Parses all <tileset> definitions in the TMX document.
     * Supports both inline tilesets (defined directly in the .tmx)
     * and external tilesets (referenced via "source" attribute to a .tsx file).
     *
     * @param document The parsed TMX XML document.
     * @return A list of TiledTileset metadata objects.
     */
    private List<TiledTileset> parseTilesets(Document document)
    {
        ...
        for (int i = 0; i < tilesetNodes.getLength(); i++)
        {
            ...
            if (source != null && !source.isEmpty())
            {
                ...
                LOGGER.info("Reading external TSX: " + tsxPath + " (firstgid=" + firstGID + ")");

                TiledTileset ts = parseExternalTileset(tsxPath, firstGID);
                if (ts != null)
                {
                    tilesets.add(ts);
                    LOGGER.info("Loaded external tileset: " + ts.getSource());
                }
                else
                {
                    LOGGER.warning("Failed to resolve external tileset: " + tsxPath);
                }
            }
            else
            {
                // Inline tileset inside the TMX
                LOGGER.info("Parsing inline tileset (firstgid=" + firstGID + ")");
                TiledTileset ts = parseInlineTileset(tilesetElement, firstGID);
                if (ts != null)
                {
                    tilesets.add(ts);
                    LOGGER.info("Loaded inline tileset: " + ts.getSource());
                }
            }
        }
    ...
    /**
     * Parses an inline <tileset> directly defined in the TMX file.
     *
     * @param tilesetElement The <tileset> XML element.
     * @param firstGID       The first global tile ID.
     * @return A TiledTileset metadata object, or null if parsing failed.
     */
    private TiledTileset parseInlineTileset(Element tilesetElement, int firstGID)
    {
        try
        {
            int tileWidth = getIntAttribute(tilesetElement, "tilewidth");
            int tileHeight = getIntAttribute(tilesetElement, "tileheight");
            int tileCount = getIntAttribute(tilesetElement, "tilecount");
            int columns = getIntAttribute(tilesetElement, "columns");

            Element imageElement = (Element) tilesetElement.getElementsByTagName("image").item(0);
            String imageSource = (imageElement != null) ? imageElement.getAttribute("source") : "";

            return new TiledTileset(resourceBasePath + imageSource,
                    firstGID, tileWidth, tileHeight, tileCount, columns);
        }
        catch (Exception e)
        {
            LOGGER.log(Level.SEVERE, "Failed to parse inline tileset", e);
            return null;
        }
    }
     ...
     *
     * @param tsxPath  Path to the TSX resource (relative to resourceBasePath).
     * @param firstGID The first global tile ID.
     * @return A TiledTileset metadata object, or null if parsing failed.
     */
    private TiledTileset parseExternalTileset(String tsxPath, int firstGID)
    {
        try (InputStream tsxStream = getClass().getClassLoader().getResourceAsStream(tsxPath))
        {
            if (tsxStream == null)
            {
                LOGGER.severe("TSX file not found on classpath: " + tsxPath);
                return null;
            }

            DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
            DocumentBuilder builder = factory.newDocumentBuilder();
            Document tsxDoc = builder.parse(tsxStream);
